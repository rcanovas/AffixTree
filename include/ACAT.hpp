...
/*! \file ACAT.hpp
    \brief ACAT.hpp contains an implementation of the Asynchronous Compressed Affix Array.
           The implementations of the Compressed Suffix Tree must use Suffix Array
           intervals as nodes.
    \author Rodrigo Canovas
*/
#ifndef ASYNC_COMP_AFFIX_TREE
#define ASYNC_COMP_AFFIX_TREE
...
namespace atds {
    ...
    public: //rename some variable types
        ...
        typedef async_node<s_node, size_type>                       node_type;
        typedef cat_tag                                             index_category;

    private: //variables
        //bool sample; //still need to think how to do this.
        //sdsl::bit_vector afl_sample[2]; // need to support rank queries and access
        cst_type            CST[2];
    ...
    public: //methods and constructors
        ...
        ACAT(sdsl::cache_config &config, sdsl::cache_config &rev_config);
        ...
        serialize(std::ostream &out, sdsl::structure_tree_node *v = NULL, std::string name = "") const {
            ...
            size_type space_aflink = 0;
            written_bytes += CST[0].serialize(out, child, "cst");
            space_aflink += affixtab[0].serialize(out, child, "affixtab");
            //written_bytes += affixtab[0].serialize(out, child, "affixtab");
            written_bytes += CST[1].serialize(out, child, "rev_cst");
            space_aflink += affixtab[1].serialize(out, child, "rev_affixtab");
            //written_bytes += affixtab[1].serialize(out, child, "rev_affixtab");
            written_bytes += space_aflink;
            std:: cout << "Space used by afftab: " << space_aflink * 1.0 / n_text << "n bytes"  << std::endl;
        ...
        get_reverse(const node_type &v) const {
            ...
            if (is_leaf(v)) {
                ...
                size_type l = depth(v);
                size_type k = l - v.l;
                rev = node_type(CST[1 - v.dir].node(affixtab[v.dir][n_home], affixtab[v.dir][n_home] + length),
                                l, k, 1 - v.dir);
            }
            return rev;
        }
    ...
    private:
        ...
        create_affixlinktab(sdsl::cache_config &config, size_type index){
            ...
            affixtab[index] = sdsl::int_vector<>(n_text, n_text, min_width);
            affixtab[index][n_text - 1] = 0; //set the root link
            std::cout << "generate the array" << std::endl;
            node_type v = root(index);
            //traverse_alink(v);
            traverse_alink_louds(v);
            std::cout << "Finished ALINK " << index << std::endl;
        }

        void
        traverse_alink(node_type &v) {
            if (is_leaf(v))  // if v is a leave, v has no child
                return;
            node_type r_node, c_node;
            size_type h = 0;
            for(auto& child: CST[v.dir].children(v.node)) {
                c_node = node_type(child, 0, 0, v.dir);
                if (!is_leaf(c_node)) {
                    h = home(c_node);
                    if (affixtab[v.dir][h] == n_text) {
                        r_node = reverse_interval(c_node, h);
                        affixtab[v.dir][h] = lb(r_node);
                    }
                    traverse_alink(c_node);
                }
            }
        }

        void
        traverse_alink_louds(node_type &v) {
            if (is_leaf(v))  // if v is a leave, v has no child
                return;
            s_node tree_node;
            node_type r_node, c_node;
        ...
        node_type
        reverse_interval(node_type &v, size_type n_home) {
            node_type rev, sl_node;
            char_type c;
            size_type length = rb(v) - lb(v);
            if (affixtab[v.dir][n_home] != n_text) {
                rev = node_type(CST[1 - v.dir].node(affixtab[v.dir][n_home], affixtab[v.dir][n_home] + length),
                                0,0, 1 - v.dir);
            }
            else {
    ...
    ACAT<t_cst, t_width>::ACAT(sdsl::cache_config &config, sdsl::cache_config &rev_config) {
        {
        ...
        }
        {
            ...
            load_from_cache(CST[1], std::string(sdsl::conf::KEY_CST) + "_" + sdsl::util::class_to_hash(CST[1]), rev_config);
        }
