...
/*! \file ACATN.hpp
    \brief ACATN.hpp contains an implementation of the Asynchronous Compressed Affix Tree Non-sampled,
            (which does not uses a affix array).
            Implementation based on the work "Multi-Pattern Matching with Bidirectional Indexes"
            by S. Gog et al. (2014)
    \author Rodrigo Canovas
*/

#ifndef AFFIXTREE_ACATN_HPP
#define AFFIXTREE_ACATN_HPP
...
namespace atds {
    ...
    public: //rename some variable types
        ...
        typedef sdsl::rmq_succinct_sct<0>                           rmax_type;
    ...
    public: //methods and constructors
        ...
        ACATN(sdsl::cache_config &config, sdsl::cache_config &rev_config);
        ...
        get_reverse(const node_type &v) const {
            ...
            if (is_leaf(v)) {
                ...
                auto q = rmax[v.dir](left, right);
                auto vlb = CST[1-v.dir].select_leaf(CST[1-v.dir].csa.isa[n-2-CST[v.dir].csa[p] - (l-1)] + 1);
                auto vrb = CST[1-v.dir].select_leaf(CST[1-v.dir].csa.isa[n-2-CST[v.dir].csa[q] - (l-1)] + 1);
                auto v_rev = CST[1-v.dir].lca(vlb,vrb);
                std::cout << v.node << "-->" << v_rev << std::endl;
    ...
    ACATN<t_cst, t_width>::ACATN(sdsl::cache_config &config, sdsl::cache_config &rev_config) {
        {
